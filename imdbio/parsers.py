...
def parse_json_title_media(raw_json) -> TitleMediaGallery:
    images = raw_json.get("images") or {}
    if not images:
        return TitleMediaGallery(total=0, items=[])
    ...
    return TitleMediaGallery(total=images.get("total", 0), items=items)
