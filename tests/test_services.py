...
def test_http_error_raised_on_non_200(monkeypatch):
    ...
    assert isinstance(err, ImdbioError)
...
def test_waf_error_is_subclass_of_http_error():
    ...
    assert issubclass(HTTPError, ImdbioError)
    assert issubclass(GraphQLError, ImdbioError)
    assert issubclass(ParseError, ImdbioError)
