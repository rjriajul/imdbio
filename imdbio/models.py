...
class TitleMediaItem(BaseModel):
    id: str
    url: str
...
class TitleMediaGallery(BaseModel):
    total: int = 0
    items: List[TitleMediaItem] = Field(default_factory=list)
...
class InfoSeries(BaseModel):
    ...
    def filter_years(cls, value):
        if value is None:
            return []
        return [
            str(y) for y in value if y is not None and (
                (isinstance(y, str) and len(y) == 4 and y.isdigit()) or isinstance(y, int)
            )
        ]
