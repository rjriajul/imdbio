...
def get_media_gallery(imdb_id: str, locale: Optional[str] = None) -> TitleMediaGallery:
    imdb_id, lang = normalize_imdb_id(imdb_id, locale)
    raw_json = _get_extended_title_info(imdb_id, lang)
    if not raw_json:
        logger.warning("No media gallery found for title %s", imdb_id)
        return TitleMediaGallery()
    gallery = parse_json_title_media(raw_json)
    logger.debug("Fetched %d media items for title %s", len(gallery.items), imdb_id)
    return gallery
